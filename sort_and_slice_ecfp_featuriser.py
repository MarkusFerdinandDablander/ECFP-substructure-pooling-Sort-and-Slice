...
def construct_sort_and_slice_ecfp_featuriser(mols_train, 
    ...
    """
    Creates a function "ecfp_featuriser" that maps RDKit mol objects to vectorial ECFP fingerprints pooled via a trained Sort & Slice operator (instead of traditional hash-based folding for substructure pooling).
    ...
    # create substructure enumerator function that maps a mol object to a dictionary whose keys are the integer substructure identifiers in mol and whose values are the associated substructure counts (i.e., how often each substructure appears in mol)
