...
def inspect_data_split_dictionary(data_split_dictionary, y_mmps):
    ...
    df = pd.DataFrame(columns = ["m", 
                                 ...
                                 "MMP: two_out_pos",
                                 "MMP: two_out_neg",
                                 "MMP: two_out_seen_core_pos",
                                 "MMP: two_out_seen_core_neg",
                                 "MMP: two_out_unseen_core_pos",
                                 "MMP: two_out_unseen_core_neg"])
    ...
    for (m, k) in data_split_dictionary.keys():
        ...
        df.loc[len(df)] = [m, 
                           ...
                           list(y_mmps[ind_two_out_seen_core_mmps]).count(1),
                           list(y_mmps[ind_two_out_seen_core_mmps]).count(0),
    ...
    df.loc[len(df)] = ["avg", 
                       "avg", 
                       np.mean(df["Mol: train"].values),
                       np.mean(df["Mol: test"].values),
                       np.mean(df["MMP: zero_out_pos"].values),
                       np.mean(df["MMP: zero_out_neg"].values),
                       np.mean(df["MMP: one_out_pos"].values),
                       np.mean(df["MMP: one_out_neg"].values),
                       np.mean(df["MMP: two_out_pos"].values),
                       np.mean(df["MMP: two_out_neg"].values),
                       np.mean(df["MMP: two_out_seen_core_pos"].values),
                       np.mean(df["MMP: two_out_seen_core_neg"].values),
                       np.mean(df["MMP: two_out_unseen_core_pos"].values),
                       np.mean(df["MMP: two_out_unseen_core_neg"].values)]
    
    # display dataframe
    display(df)
    
    # compute ratios of positives to negatives in mmp subgroups
    ratios = []

    for (kk, (v_1, v_2)) in enumerate(zip(list(df.iloc[-1].values)[4:-1], list(df.iloc[-1].values)[5:])):

        if kk % 2 == 0:

            ratios.append(v_2/v_1)

    print("zero_out ratio = ", ratios[0])
    print("one_out ratio = ", ratios[1])
    print("two_out ratio = ", ratios[2])
    print("two_out_seen_core ratio = ", ratios[3])
    print("two_out_unseen_core ratio = ", ratios[4])
