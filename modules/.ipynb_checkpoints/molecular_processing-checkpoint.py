...
# other modules
#from .graph_theory import subgraph_isomorphism_vertex_counts, create_molecular_graph, create_circle_graph, create_line_graph, create_star_graph
...
def ecfp_bitstrings_from_smiles(smiles,
                                radius,
                                bitstring_length,
                                use_bond_invs,
                                mol_to_invs_function):


    mol = Chem.MolFromSmiles(smiles)
    fingerprint = np.array(Chem.rdMolDescriptors.GetMorganFingerprintAsBitVect(mol,
                                                                               radius = radius,
...
def ring_isomorphism_invariants(mol, ring_sizes = [5, 6, 7], induced = True):
    
    M = create_molecular_graph(mol)
    
    counts = np.empty([M.num_vertices(), 0], dtype = int)
    
    for k in ring_sizes:
        
        C = create_cycle_graph(k)
        counts = np.append(counts, subgraph_isomorphism_vertex_counts(M, C, induced = induced), axis = 1)
    
    invs = [hash(tuple(counts_for_atom_i)) % 10000001 for counts_for_atom_i in counts]
    
    return invs



def line_isomorphism_invariants(mol, line_sizes = [3, 4, 5], induced = True):
    ...
    for k in line_sizes:
        ...
        L = create_line_graph(k)
        counts = np.append(counts, subgraph_isomorphism_vertex_counts(M, L, induced = induced), axis = 1)
...
def star_isomorphism_invariants(mol, star_sizes = [3, 4, 5], induced = True):
    ...
    for k in star_sizes:
        ...
        S = create_star_graph(k)
        counts = np.append(counts, subgraph_isomorphism_vertex_counts(M, S, induced = induced), axis = 1)
...
def tan_sim(fp_1, fp_2):
    
    fp_1_intersect_fp_2 = np.dot(fp_1, fp_2)
    
    fp_1_sum = np.sum(fp_1)
    fp_2_sum = np.sum(fp_2)
    
    tanimoto_similarity = fp_1_intersect_fp_2/(fp_1_sum + fp_2_sum - fp_1_intersect_fp_2)
    
    return tanimoto_similarity



def tan_sim_vector(fp, fp_base):
    
    return np.array([tan_sim(fp, fp_current) for fp_current in fp_base])
