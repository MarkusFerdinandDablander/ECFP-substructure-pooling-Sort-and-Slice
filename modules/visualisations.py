...
def visualise_ecfp_bar_charts(dataset_name, split_type, metric, title = "", y_unit = ""):
    ...
    # add title
    fig.suptitle(title, fontsize = 14, x = 0.52, y = 1.005, fontweight = "bold")
    ...
    for (i, ml) in enumerate(["rf", "mlp"]):
        ...
        # set title
        if ml == "rf":
            ax.set_title("Random Forest", fontsize = 14, pad = 15)
            
        elif ml == "mlp":
            ax.set_title("\n Multilayer Perceptron", fontsize = 14, pad = 15)
...
def visualise_ecfp_hyperparameter_space_boxplots_one_dataset(dataset_name, metric, show_x_ticks = True, show_legend = True):
    ...
    for (j_1, split_type) in enumerate(["rand", "scaff"]):
        for (j_2, ml) in enumerate(["rf", "mlp"]):
            ...
            # set y-label
            if j_1 + j_2 == 0:
                ax.set_ylabel(metric, fontsize = 10, labelpad = 10, fontweight = "normal")
